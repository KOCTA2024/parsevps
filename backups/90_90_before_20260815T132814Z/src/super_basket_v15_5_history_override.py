...
def classify_candidate(
    *, p_final: float, p_score: float, edge: float, edge_z: float, odds: Optional[float],
    ...
    real bookmaker line is actionable.  Exact-score delta >=4 is mandatory;
    history and scenario must each be >=60% for any money signal.  FAKE/stat/Q4
...
def telegram_message(result: dict[str, Any]) -> str:
    ...
    lines.extend([
        "",
        "Exact-score є якорем напрямку. Для ставки потрібні real line, delta >=4 та P_history/P_scenario >=60%.",
