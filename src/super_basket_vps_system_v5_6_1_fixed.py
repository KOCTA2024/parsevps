#!/usr/bin/env python3
"""SUPER_BASKET VPS ACTION SYSTEM v5.6.1 - single-file edition.
...
def _parse_history_date(value: Any) -> Optional[datetime]:
    ...
    for candidate in (normalized, normalized.replace('/', '-')):
        try:
            ...
            pass
    for fmt in ('%d.%m.%Y %H:%M:%S', '%d.%m.%Y %H:%M', '%d.%m.%Y', '%d-%m-%Y %H:%M', '%d-%m-%Y', '%Y-%m-%d %H:%M:%S', '%Y-%m-%d'):
        try:
...
def adapt_match(source: dict[str, Any], config: dict[str, Any], strict: bool=False) -> dict[str, Any]:
    ...
    reference_candidates = [
        first(match, ['date', 'start_time', 'scheduled_at', 'snapshot_at']),
        source.get('snapshot_at'),
        source.get('generated_at'),
        (source.get('meta') or {}).get('generated_at') if isinstance(source.get('meta'), dict) else None,
        ((source.get('bookmaker_lines') or {}).get('_source_meta') or {}).get('fetchedAt')
            if isinstance(source.get('bookmaker_lines'), dict) else None,
        first(raw_main, ['dt', 'date', 'start_time']),
    ]
    reference_datetime = next((_parse_history_date(value) for value in reference_candidates if _parse_history_date(value) is not None), None)
    return {
        'match_id': current_id,
...
def _parser_projection_components(market: dict[str, Any], canonical: dict[str, Any], clock: dict[str, float]) -> dict[str, dict[str, Any]]:
    """Read parser projections without letting a stale/secondary estimate dominate P_live.

    The parser may explicitly declare which projection is the main live estimate.  v5.5
    always trusted ``live_calibrated`` and rejected ``segment_projection`` when the two
    diverged, even when the payload said the opposite.  v5.6 honours that priority and
    treats pre-match values as diagnostics, not as live evidence.
    """
    ...
    if market_type in {'MATCH_TOTAL', 'TEAM_IT_MATCH'}:
        ...
        priority = projections.get('projection_priority') if isinstance(projections.get('projection_priority'), dict) else {}
        main_name = str(priority.get('main_for_p_live') or '').strip().lower()
        suffix = f'{team_side}_final' if team_side else 'total'
        live_value = to_number(live.get(suffix))
        segment_value_ = to_number(segment.get(suffix))
        pre_value = pre.get(suffix)
        live_reason = None if snapshot_ok and live.get('valid', True) else 'PARSER_PROJECTION_STALE_OR_INVALID'
        segment_reason = None if snapshot_ok else 'PARSER_PROJECTION_STALE_OR_INVALID'
        live_weight = 0.16
        segment_weight = 0.16
        if 'segment' in main_name:
            segment_weight, live_weight = 0.22, 0.07
        elif 'live_calibrated' in main_name or main_name == 'live':
            live_weight, segment_weight = 0.22, 0.07
        elif segment_value_ is not None:
            # Segment projection is normally the closer representation of the current
            # score/clock state; live_calibrated remains a secondary independent check.
            segment_weight, live_weight = 0.18, 0.09
        if live_value is not None and segment_value_ is not None:
            divergence = abs(segment_value_ - live_value)
            divergence_limit = max(18.0, 0.14 * max(abs(segment_value_), abs(live_value)))
            if divergence > divergence_limit:
                if 'live_calibrated' in main_name or main_name == 'live':
                    segment_reason = 'PARSER_SEGMENT_DIVERGES_FROM_PRIORITY_LIVE_CALIBRATED'
                else:
                    live_reason = 'PARSER_LIVE_CALIBRATED_DIVERGES_FROM_PRIORITY_SEGMENT'
        add('projection_parser_live_calibrated', live_value, live_reason, suggested_weight=live_weight)
        add('projection_parser_segment', segment_value_, segment_reason, suggested_weight=segment_weight)
        add('projection_parser_pre_match', pre_value, 'PRE_MATCH_PROJECTION_NOT_LIVE_EVIDENCE', suggested_weight=0.0)
        ...
        if team_side:
            ...
            key = 'total_center'
        add('projection_parser_current_quarter', quarter_projection.get(key), suggested_weight=0.20)
    ...
    if market_type in {'MATCH_TOTAL', 'H1_TOTAL', 'H2_TOTAL', 'CURRENT_QUARTER_TOTAL'}:
        ...
        weight = 0.10 if readiness == 'READY' else 0.05 if readiness == 'REVIEW_REQUIRED' else 0.0
        add('projection_coursework_remaining', value, reason, suggested_weight=weight)
    return result

def _piecewise_layoff_strength(days_since_last: Optional[float]) -> float:
    if days_since_last is None or days_since_last <= 14:
        return 0.0
    if days_since_last <= 30:
        return 0.15 * (days_since_last - 14) / 16
    if days_since_last <= 60:
        return 0.15 + 0.30 * (days_since_last - 30) / 30
    if days_since_last <= 120:
        return 0.45 + 0.30 * (days_since_last - 60) / 60
    return min(1.0, 0.75 + 0.25 * (days_since_last - 120) / 180)


def calculate_history_recency(canonical: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    """Estimate how much old history should be discounted after breaks/season resets."""
    settings = config.get('history_recency', {})
    gap_days = float(settings.get('season_gap_days', 40.0))
    recent_season_max_games = int(settings.get('recent_season_max_games', 8))
    reference = _parse_history_date(canonical.get('reference_datetime'))

    def profile(pool_name: str) -> dict[str, Any]:
        games = canonical.get('history', {}).get(pool_name) or []
        dates = sorted({dt for dt in (_parse_history_date(game.get('date')) for game in games) if dt is not None}, reverse=True)
        newest = dates[0] if dates else None
        days_since_last = max(0.0, (reference - newest).total_seconds() / 86400.0) if reference and newest else None
        layoff_strength = _piecewise_layoff_strength(days_since_last)
        first_large_gap = None
        games_since_gap = None
        for index in range(min(len(dates) - 1, 16)):
            gap = (dates[index] - dates[index + 1]).total_seconds() / 86400.0
            if gap >= gap_days:
                first_large_gap = gap
                games_since_gap = index + 1
                break
        explicit_starts = []
        for game in games[:recent_season_max_games + 2]:
            raw = game.get('raw') if isinstance(game.get('raw'), dict) else {}
            parsed = _parse_history_date(first(raw, ['current_season_start_dt', 'season_start_dt']))
            if parsed is not None:
                explicit_starts.append(parsed)
        explicit_start = max(explicit_starts) if explicit_starts else None
        explicit_days = max(0.0, (reference - explicit_start).total_seconds() / 86400.0) if reference and explicit_start else None
        inferred_recent_season = first_large_gap is not None and games_since_gap is not None and games_since_gap <= recent_season_max_games
        explicit_recent_season = explicit_days is not None and explicit_days <= float(settings.get('recent_season_window_days', 75.0))
        season_strength = 0.0
        if inferred_recent_season:
            scarcity = (recent_season_max_games - games_since_gap + 1) / max(1, recent_season_max_games)
            season_strength = min(0.82, 0.35 + 0.32 * scarcity + 0.15 * min(1.0, first_large_gap / 90.0))
        if explicit_recent_season:
            season_strength = max(season_strength, min(0.85, 0.50 + 0.35 * (1.0 - explicit_days / max(1.0, float(settings.get('recent_season_window_days', 75.0))))))
        strength = max(layoff_strength, season_strength)
        return {
            'pool': pool_name,
            'dated_games': len(dates),
            'newest_game': newest.isoformat() if newest else None,
            'days_since_last': days_since_last,
            'layoff_strength': layoff_strength,
            'first_large_gap_days': first_large_gap,
            'games_since_gap': games_since_gap,
            'recent_season_detected': bool(inferred_recent_season or explicit_recent_season),
            'season_strength': season_strength,
            'strength': strength,
        }

    team_a = profile('team_a')
    team_b = profile('team_b')
    dated_coverage = min(1.0, (team_a['dated_games'] + team_b['dated_games']) / max(1.0, float(canonical.get('data_gate', {}).get('pooled_n') or 1)))
    raw_strength = max(float(team_a['strength']), float(team_b['strength']))
    strength = raw_strength * dated_coverage
    return {
        'reference_datetime': reference.isoformat() if reference else None,
        'team_a': team_a,
        'team_b': team_b,
        'dated_coverage': dated_coverage,
        'strength': max(0.0, min(1.0, strength)),
        'active': bool(reference and dated_coverage >= 0.25 and strength >= 0.10),
        'reason_codes': [
            code for code, active in (
                ('TEAM_A_LONG_BREAK_OR_NEW_SEASON', team_a['strength'] >= 0.35),
                ('TEAM_B_LONG_BREAK_OR_NEW_SEASON', team_b['strength'] >= 0.35),
            ) if active
        ],
    }


def _dynamic_regression_weights(base: dict[str, float], current_key: str, historical_keys: list[str], recency_strength: float, shift: float) -> dict[str, float]:
    weights = {key: max(0.0, float(value)) for key, value in base.items()}
    if current_key not in weights or recency_strength <= 0:
        total = sum(weights.values()) or 1.0
        return {key: value / total for key, value in weights.items()}
    available_shift = min(shift * recency_strength, sum(weights.get(key, 0.0) for key in historical_keys))
    donor_total = sum(weights.get(key, 0.0) for key in historical_keys)
    if donor_total > 0 and available_shift > 0:
        for key in historical_keys:
            donor = weights.get(key, 0.0)
            weights[key] = max(0.0, donor - available_shift * donor / donor_total)
        weights[current_key] = weights.get(current_key, 0.0) + available_shift
    total = sum(weights.values()) or 1.0
    return {key: value / total for key, value in weights.items()}


def _robust_projection_mean(items: list[tuple[str, float, float]], base_sigma: float) -> tuple[float, set[str], float]:
    if not items:
        return 0.0, set(), 0.0
    ...
    threshold = max(base_sigma * 2.15, mad * 3.5, 4.0)
    included = [item for item in items if abs(item[1] - center) <= threshold]
    excluded = {name for name, value, _ in items if abs(value - center) > threshold}
    if len(included) < 2:
        included, excluded = list(items), set()
    total_weight = sum(max(0.0, weight) for _, _, weight in included)
    if total_weight <= 0:
        projection = statistics.median(value for _, value, _ in included)
    else:
        projection = sum(value * max(0.0, weight) for _, value, weight in included) / total_weight
    included_values = [value for _, value, _ in included]
    spread = statistics.pstdev(included_values) if len(included_values) >= 2 else 0.0
    return projection, excluded, spread


def _p_live_for_side(projection: float, line: float, side: str, sigma: float, reliability: float=1.0, edge_shrink: float=1.0) -> dict[str, float]:
    """Direction-safe live probability. UNDER is high only when projection < line."""
    side_upper = str(side or '').upper()
    raw_edge = projection - line if side_upper == 'OVER' else line - projection
    effective_edge = raw_edge * max(0.0, min(1.0, edge_shrink))
    safe_sigma = max(1e-6, float(sigma))
    raw_probability = normal_cdf(effective_edge / safe_sigma)
    rel = max(0.0, min(1.0, reliability))
    calibrated = 0.5 + rel * (raw_probability - 0.5)
    return {
        'raw_edge': raw_edge,
        'effective_edge': effective_edge,
        'z_score': effective_edge / safe_sigma,
        'raw_probability': raw_probability,
        'probability': max(0.01, min(0.99, calibrated)),
    }


def calculate_live_projection(market: dict[str, Any], canonical: dict[str, Any], history: dict[str, Any], scenario: dict[str, Any], config: dict[str, Any], stat: Optional[dict[str, Any]]=None, recency: Optional[dict[str, Any]]=None) -> dict[str, Any]:
    clock = _segment_clock(market, canonical)
    elapsed_minutes = canonical['elapsed_game_seconds'] / 60
    recency = recency or calculate_history_recency(canonical, config)
    recency_strength = float(recency.get('strength') or 0.0)
    ...
    if history_projection is not None and scenario_center is not None and clock['full_seconds']:
        ...
        scenario_credibility = max(0.0, min(0.55, float(scenario.get('scenario_credibility') or 0.0)))
    ...
    rate_weights = _dynamic_regression_weights(
        {'current': 0.48, 'previous': 0.27, 'history': 0.25},
        'current', ['history'], recency_strength, shift=0.16,
    )
    segment_rates = [(current_rate, rate_weights['current']), (previous_rate, rate_weights['previous']), (historical_rate, rate_weights['history'])]
    ...
    stat_adjusted = None
    stat_details: dict[str, Any] = {'team_metrics': team_metrics, 'game_possessions': game_possessions, 'combined_ppp': combined_ppp, 'history_recency': recency}
    if game_possessions is not None and elapsed_minutes > 0:
        ...
        regression = config['projection']['regression']
        pace_weights = _dynamic_regression_weights(
            {'current': regression['current_pace'], 'history': regression['history_pace'], 'scenario': regression['scenario_pace']},
            'current', ['history', 'scenario'], recency_strength, shift=0.22,
        )
        blended_future_pace = current_pace * pace_weights['current'] + historical_pace * pace_weights['history'] + scenario_pace * pace_weights['scenario']
        future_possessions = blended_future_pace * remaining_minutes
        regressed: dict[str, float] = {}
        ppp_weights = _dynamic_regression_weights(
            {'current': regression['current_ppp'], 'offense': regression['historical_offense_ppp'], 'allowed': regression['opponent_allowed_ppp'], 'scenario': regression['scenario_ppp']},
            'current', ['offense', 'allowed', 'scenario'], recency_strength, shift=0.20,
        )
        for side, offense, opponent_allowed in (('home', home_offense, away_allowed), ('away', away_offense, home_allowed)):
            ...
            available = [(base_values[key], ppp_weights[key]) for key in base_values if base_values[key] is not None]
        ...
        for rule_id, active, config_key in adjustment_rules:
            if not active:
                continue
            delta_value = float(adjustments.get(config_key, 0.0))
            adjustment_rate += delta_value
            adjustment_events.append({'rule_id': rule_id, 'delta': delta_value})
        adjustment_rate = max(-0.07, min(0.07, adjustment_rate))
        ...
        stat_details.update({'current_pace': current_pace, 'historical_pace': historical_pace, 'scenario_pace': scenario_pace, 'pace_weights': pace_weights, 'blended_future_pace': blended_future_pace, 'future_possessions': future_possessions, 'regressed_ppp': regressed, 'ppp_weights': ppp_weights, 'adjustment_rate': adjustment_rate, 'adjustment_events': adjustment_events})
    ...
    component_values = {
        ...
        'projection_segment': segment_projection,
        'projection_history': history_projection,
        'projection_scenario': scenario_projection,
        'projection_stat_adjusted': stat_adjusted,
    }
    component_weights = {
        'projection_simple': 0.07,
        'projection_segment': 0.36,
        # History/scenario remain visible for audit but are not fed into P_live.
        # They enter P_raw once through P_hist/P_scenario, avoiding double counting.
        'projection_history': 0.0,
        'projection_scenario': 0.0,
        'projection_stat_adjusted': 0.34,
    }
    for key, item in parser_components.items():
        component_values[key] = item.get('value') if item.get('available') else None
        component_weights[key] = float(item.get('suggested_weight') or 0.0)

    line = float(market['line'])
    base_sigma = _stage_sigma(market['market_type'], canonical['stage'], config)
    items = [(key, float(value), max(0.0, component_weights.get(key, 0.0))) for key, value in component_values.items() if value is not None and component_weights.get(key, 0.0) > 0]
    if items:
        projection_used, robust_excluded, component_spread = _robust_projection_mean(items, base_sigma)
    else:
        projection_used, robust_excluded, component_spread = line, set(), 0.0

    components: dict[str, dict[str, Any]] = {}
    for key, original_value in component_values.items():
        parser_exclusion = parser_components.get(key, {}).get('exclusion_reason')
        included = original_value is not None and component_weights.get(key, 0.0) > 0 and key not in robust_excluded and parser_exclusion is None
        components[key] = {
            'value': parser_components.get(key, {}).get('value', original_value),
            'weight': component_weights.get(key, 0.0),
            'available': original_value is not None,
            'included': included,
            'exclusion_reason': 'ROBUST_OUTLIER_FROM_LIVE_ANCHOR' if key in robust_excluded else parser_exclusion or ('ZERO_WEIGHT_DIAGNOSTIC' if original_value is not None and component_weights.get(key, 0.0) <= 0 else 'UNAVAILABLE' if original_value is None else None),
        }

    calibration_cfg = projection_cfg.get('p_live_calibration', {})
    stat_support = str(canonical.get('stat_support') or 'OFF')
    sigma_multiplier = 1.0
    if stat_support == 'OFF':
        sigma_multiplier *= float(calibration_cfg.get('no_stat_sigma_multiplier', 1.20))
    elif stat_support == 'LIMITED':
        sigma_multiplier *= float(calibration_cfg.get('limited_stat_sigma_multiplier', 1.10))
    spread_factor = float(calibration_cfg.get('component_spread_sigma_factor', 0.55))
    effective_sigma = math.sqrt((base_sigma * sigma_multiplier) ** 2 + (component_spread * spread_factor) ** 2)
    stage_reliability = float((calibration_cfg.get('reliability_by_stage') or {}).get(canonical['stage'], 0.84))
    stat_reliability = {'ON': 1.0, 'LIMITED': 0.92, 'OFF': 0.80}.get(stat_support, 0.80)
    included_count = sum(1 for row in components.values() if row.get('included'))
    component_reliability = min(1.0, 0.62 + 0.08 * included_count)
    disagreement_reliability = 1.0 / (1.0 + component_spread / max(1.0, base_sigma * 2.0))
    reliability = max(0.48, min(0.98, stage_reliability * stat_reliability * component_reliability * disagreement_reliability))
    anchor_by_stage = calibration_cfg.get('line_anchor_by_stage') or {}
    edge_shrink = float(anchor_by_stage.get(canonical['stage'], 0.86))
    if stat_support == 'OFF':
        edge_shrink *= float(calibration_cfg.get('no_stat_edge_multiplier', 0.88))
    live_probability = _p_live_for_side(projection_used, line, market['side'], effective_sigma, reliability, edge_shrink)

    control_values = [value for value in (segment_projection, stat_adjusted, simple) if value is not None]
    control = statistics.median(control_values) if control_values else projection_used
    return {
        ...
        'projection_simple': simple,
        'projection_segment': segment_projection,
        'projection_model_live': segment_projection,
        'projection_history': history_projection,
        'projection_scenario': scenario_projection,
        'scenario_projection_method': scenario_projection_method,
        'projection_stat_adjusted': stat_adjusted,
        'projection_control': control,
        'projection_used': projection_used,
        'Projection_used': projection_used,
        'line': line,
        'line_edge': live_probability['raw_edge'],
        'line_edge_effective': live_probability['effective_edge'],
        ...
        'z_score': live_probability['z_score'],
        'p_live_raw': live_probability['raw_probability'],
        'p_live': live_probability['probability'],
        'stat_projection_details': stat_details,
        'history_recency': recency,
    }
...
def _set_live_weight(weights: dict[str, float], target_live: float, hist_factor: float=1.0, scenario_factor: float=1.0) -> dict[str, float]:
    target = max(0.0, min(0.90, float(target_live)))
    non_live = max(0.0, 1.0 - target)
    hist_base = max(0.0, float(weights.get('hist', 0.0))) * max(0.0, hist_factor)
    scenario_base = max(0.0, float(weights.get('scenario', 0.0))) * max(0.0, scenario_factor)
    denominator = hist_base + scenario_base
    if denominator <= 0:
        return {'hist': non_live, 'scenario': 0.0, 'live': target}
    return {
        'hist': non_live * hist_base / denominator,
        'scenario': non_live * scenario_base / denominator,
        'live': target,
    }


def calculate_dynamic_stage_weights(
    market: dict[str, Any],
    canonical: dict[str, Any],
    ...
    settings = config.get('dynamic_weights', {})
    audit: dict[str, Any] = {'base': deepcopy(base_weights), 'adjustments': []}
    if canonical.get('stage') == 'PRE_MATCH':
        audit['effective'] = deepcopy(weights)
        return weights, audit

    recency_strength = float(recency.get('strength') or 0.0)
    stage_max = float((settings.get('recency_max_live_weight') or {}).get(canonical.get('stage'), 0.72))
    if recency.get('active') and recency_strength > 0 and stage_max > weights.get('live', 0.0):
        before = deepcopy(weights)
        target = weights['live'] + recency_strength * (stage_max - weights['live'])
        weights = _set_live_weight(weights, target, hist_factor=1.0 - 0.20 * recency_strength, scenario_factor=1.0 - 0.38 * recency_strength)
        audit['adjustments'].append({'rule_id': 'HISTORY_RECENCY_SHIFT', 'strength': recency_strength, 'before': before, 'after': deepcopy(weights), 'recency': recency})

    p_hist = float(history.get('p_hist') or 0.5)
    p_live = float(live.get('p_live') or 0.5)
    line_edge = float(live.get('line_edge') or 0.0)
    history_threshold = float(settings.get('history_conflict_threshold', 0.82))
    if p_hist >= history_threshold and p_live < 0.5 and line_edge < 0:
        before = deepcopy(weights)
        hist_strength = min(1.0, max(0.0, (p_hist - history_threshold) / max(1e-6, 1.0 - history_threshold)))
        opposition_strength = min(1.0, max(0.0, (0.5 - p_live) / 0.22))
        edge_strength = min(1.0, abs(line_edge) / max(1.0, _strong_edge_threshold(market['market_type'], config)))
        conflict_strength = min(1.0, 0.40 * hist_strength + 0.40 * opposition_strength + 0.20 * edge_strength)
        conflict_min = float(settings.get('conflict_min_live_weight', 0.62))
        conflict_max = float(settings.get('conflict_max_live_weight', 0.75))
        target = max(weights['live'], conflict_min + conflict_strength * max(0.0, conflict_max - conflict_min))
        weights = _set_live_weight(weights, target, hist_factor=1.0, scenario_factor=0.88)
        audit['adjustments'].append({'rule_id': 'HISTORY_LIVE_OPPOSITION_SHIFT', 'strength': conflict_strength, 'before': before, 'after': deepcopy(weights), 'p_hist': p_hist, 'p_scenario': scenario.get('p_scenario'), 'p_live': p_live, 'line_edge': line_edge})
...
def _low_band_risk_allowed(
    probability: float,
    p_hist: Optional[float],
    p_live: Optional[float]=None,
) -> bool:
    """Allow RISK in the 60-64.99% band only under the configured ceiling rule.

    User rule v5.6.1:
    - 0.60 <= P_final < 0.65;
    - P_live must be strictly below 0.80;
    - P_hist must be strictly below 0.85.

    Missing P_live or P_hist cannot satisfy the rule. Values at the ceilings
    themselves (80% and 85%) are PASS because both comparisons are strict.
    """
    return (
        0.60 <= float(probability) < 0.65
        and p_live is not None
        and float(p_live) < 0.80
        and p_hist is not None
        and float(p_hist) < 0.85
    )
...
def _verdict(
    ...
    if blockers or (
        probability < 0.65
        and not _low_band_risk_allowed(probability, p_hist, p_live)
    ):
        return 'PASS'
    if probability < 0.75:
        return 'RISK PLAY'
...
class SuperBasketCalculator:
    ...
    def evaluate_market(self, market: dict[str, Any], canonical: dict[str, Any]) -> dict[str, Any]:
        ...
        history_recency = calculate_history_recency(canonical, self.config)
        live = calculate_live_projection(market, canonical, history, scenario, self.config, projection_stat, history_recency)
        ...
        normalized_weights, dynamic_weight_audit = calculate_dynamic_stage_weights(
            market, canonical, history, scenario, live, history_recency, base_weights, self.config
        )
        ...
        if live_dominance['active']:
            ...
            normalized_weights = _set_live_weight(normalized_weights, live_weight)
            dynamic_weight_audit['adjustments'].append({
                'rule_id': 'LIVE_DOMINANCE',
                'after': deepcopy(normalized_weights),
                'details': live_dominance,
            })
            dynamic_weight_audit['effective'] = deepcopy(normalized_weights)
        ...
        if history['p_hist'] >= 0.9 and live['line_edge'] <= -strong_edge:
            blockers.append(_blocker('STRONG_HISTORY_LIVE_CONFLICT', 'History side is blocked by a strong opposite live projection', {'p_hist': history['p_hist'], 'p_live': live['p_live'], 'line_edge': live['line_edge'], 'threshold': strong_edge}))
        elif history['p_hist'] >= 0.85 and live['p_live'] < 0.42 and live['line_edge'] < 0:
            caps.append(_cap('HISTORY_LIVE_OPPOSITION_CAP', 0.64, 'High history is not allowed to lift P_final when the calibrated live projection opposes the market side', {'p_hist': history['p_hist'], 'p_live': live['p_live'], 'projection_used': live['projection_used'], 'line': market['line'], 'line_edge': live['line_edge']}))
        if market['market_type'].startswith('TEAM_IT') or market['market_type'] == 'CURRENT_QUARTER_TEAM_IT':
        ...
        p_trace = [
            ...
            _trace_step('P_LIVE', canonical['stage'] != 'PRE_MATCH', 'calibrated Phi(direction-safe edge / disagreement-adjusted sigma), shrunk toward 50% by reliability and bookmaker-line anchoring', {'projection_used': live.get('projection_used'), 'line': market['line'], 'line_edge': live.get('line_edge'), 'effective_edge': live.get('line_edge_effective'), 'sigma_base': live.get('sigma_base'), 'sigma': live.get('sigma'), 'component_spread': live.get('component_spread'), 'reliability': live.get('reliability'), 'edge_shrink': live.get('edge_shrink'), 'p_live_raw': live.get('p_live_raw'), 'scenario_projection_method': live.get('scenario_projection_method')}, None, live['p_live']),
            _trace_step('STAGE_WEIGHTS', True, 'dynamic stage weights: stale/new-season history and history-vs-live opposition move weight toward calibrated live; aligned LIVE_DOMINANCE may raise live weight to at most 0.80', {'stage': stage_key, 'base_weights': base_weights, 'effective_weights': normalized_weights, 'normalization_applied': normalization_applied, 'history_recency': history_recency, 'dynamic_weight_audit': dynamic_weight_audit, 'live_dominance': live_dominance}, None, p_raw, [item['rule_id'] for item in dynamic_weight_audit.get('adjustments', [])]),
        ...
        return {**market, 'history': history, 'scenario': scenario, 'history_recency': history_recency, 'live': live, 'stat_comparison': stat, 'q4_context': q4, 'zone_credibility': zone_credibility, 'live_dominance': live_dominance, 'weights': {'original': original_weights, 'base_normalized': base_weights, 'normalized': normalized_weights, 'normalization_applied': normalization_applied, 'dynamic_adjustment': dynamic_weight_audit, 'live_dominance_applied': live_dominance['active']}, 'p_raw': p_raw, 'router': router, 'caps': caps, 'blockers': blockers, 'hard_conflict': bool(blockers), 'p_final': p_final, 'verdict': verdict, 'p_trace': p_trace, 'strong_requirements': {'aligned': alignment, 'stat_confirmation': stat['stat_gate_status'] == 'CONFIRMED', 'sample_sufficient': sample_sufficient, 'clean': strong_clean}}
    ...
    def calculate(self, source: dict[str, Any], dispatch_threshold: Optional[float]=None, strict_schema: bool=False) -> dict[str, Any]:
        ...
        candidates = [
            ...
            if (
                ...
                or (
                    threshold <= 0.65
                    and _low_band_risk_allowed(
                        evaluation['p_final'],
                        evaluation.get('history', {}).get('p_hist'),
                        evaluation.get('live', {}).get('p_live'),
                    )
                )
            )
...
DEFAULT_CONFIG = json.loads(r"""{
  ...
  "history_recency": {
    "season_gap_days": 40,
    "recent_season_max_games": 8,
    "recent_season_window_days": 75
  },
  "dynamic_weights": {
    "history_conflict_threshold": 0.82,
    "conflict_min_live_weight": 0.62,
    "conflict_max_live_weight": 0.75,
    "recency_max_live_weight": {
      "EARLY_LIVE": 0.68,
      "HT": 0.72,
      "AFTER_3Q": 0.78,
      "Q4_CONFIRMATION": 0.82,
      "CURRENT_Q1_Q3": 0.72
    }
  },
  "projection": {
    ...
    "p_live_calibration": {
      ...
      "limited_stat_sigma_multiplier": 1.10,
      "component_spread_sigma_factor": 0.55,
      "no_stat_edge_multiplier": 0.88,
      "reliability_by_stage": {
        "PRE_MATCH": 0.55,
        "EARLY_LIVE": 0.82,
        "HT": 0.90,
        "AFTER_3Q": 0.95,
        "Q4_CONFIRMATION": 0.97,
        "CURRENT_Q1_Q3": 0.86
      },
      "line_anchor_by_stage": {
        "PRE_MATCH": 0.75,
        "EARLY_LIVE": 0.82,
        "HT": 0.88,
        "AFTER_3Q": 0.93,
        "Q4_CONFIRMATION": 0.96,
        "CURRENT_Q1_Q3": 0.86
      }
    },
    "weights": {
...
def normalized_action(
    ...
    if probability < 0.60:
        return 'PASS', 'PASS', '0%'
    if probability < 0.65:
        if _low_band_risk_allowed(probability, p_hist, p_live):
            return 'RISK', 'RISK ENTRY — LOW-BAND CEILING RULE', '10-15% live-limit'
        return 'PASS', 'PASS', '0%'
    if probability < 0.75:
...
RISK_POST_FILTER_P_LIVE_THRESHOLD = 0.90


def apply_risk_post_filter(decision: dict[str, Any]) -> dict[str, Any]:
    """Suppress only an already-formed RISK when P_live is not strictly above 90%.

    PLAY and deterministic PASS decisions bypass this delivery filter unchanged.
    ...
    result = {
        'enabled': True,
        'applicable': deterministic_action == 'RISK',
        'comparison': '>',
    ...
    if deterministic_action != 'RISK':
        return result
    if p_live is not None and p_live > RISK_POST_FILTER_P_LIVE_THRESHOLD:
        return result

    reason_code = 'RISK_POSTFILTER_P_LIVE_NOT_ABOVE_90'
    result.update({
        'passed': False,
        'filtered': True,
    ...
    decision['explanation_uk'] = (
        ...
        f'RISK post-filter: P_live {live_label}; для відправки потрібно строго більше 90%.'
    ).strip()
    decision['main_risk_uk'] = (
        'Детермінований RISK збережено для аудиту, але Telegram-сигнал '
        'відфільтровано через недостатній P_live.'
    )
    decision['trigger_uk'] = 'Для відправки RISK потрібен P_live >90%.'
    return result
...
def apply_learning_to_evaluation(evaluation: dict[str, Any], store: LearningStore, calculation: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    item['system_reason_codes'] = (
        ['RISK_60_64_P_LIVE_LT_80_P_HIST_LT_85']
        if action == 'RISK' and _low_band_risk_allowed(
            p_calibrated,
            p_hist,
            p_live,
        )
    ...
    if action == 'PASS':
        ...
        trigger = (
            ...
            if mode.upper() == 'STRICT'
                ...
                'P_live <80% та P_hist <85%, без hard blocker.'
            )
        )
...
def process_vps_match_file(
    ...
    *,
    ...
    try:
        ...
        delivery = {
            'status': (
                'SKIPPED_RISK_POSTFILTER_P_LIVE_NOT_ABOVE_90'
        ...
        if selected and decision.get('signal_id'):
            ...
            if already_sent:
                ...
                delivery = {
                    'status': 'SKIPPED_RISK_POSTFILTER_P_LIVE_NOT_ABOVE_90',
                    'sent': False,
                    'message_id': None,
                }
