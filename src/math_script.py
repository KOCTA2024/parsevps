...
MIN_VALID_MATCHES = 35
PARSEDATAFILE = [entry for entry in os.scandir(os.path.join(os.path.dirname(__file__), "data")) if entry.is_file()][0].path
print(PARSEDATAFILE)

LINES_FILE = os.path.join(os.path.dirname(__file__), "data", "lines.json", "line_result.json")
...
def parse_records(raw: list) -> dict:
    ...
    for rec in raw:
        ...
        if "LAST 35" in src_up or "LAST35" in src_up:
            if "H2H" in src_up:
                current_section = "h2h"
            else:
...
def build_hist(records: list, perspective: str, main_match_id: str) -> list:
    ...
    for rec in records:
        ...
        if row:
            rows.append(row)
    return rows[:MIN_VALID_MATCHES]  # take up to 35
...
def process_match(parsed: dict, lines_data: dict = None) -> dict:
    ...
    for rec in rows_a_raw:
        ...
        if row:
            rows_a.append(row)
    rows_a = rows_a[:MIN_VALID_MATCHES]
    ...
    for rec in rows_b_raw:
        ...
        if row:
            rows_b.append(row)
    rows_b = rows_b[:MIN_VALID_MATCHES]
...
def main():
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "result.json")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(merged, f, ensure_ascii=False, indent=2)
    print(f"[result.json written → {output_path}]", file=sys.stderr)
