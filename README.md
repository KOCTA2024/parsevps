...
## Match URL -> Last 20 + H2H + Quarter scores

This project now includes a dedicated export mode for a single Flashscore match URL.

### What it does

Given one match URL, it will:
- extract the two team pages from the match
- collect the last N finished matches for each team
- collect H2H matches by scanning one team's results and filtering by the opponent
- open every collected match via `flashscore.mobi` and extract:
  - final score
  - Q1 / Q2 / Q3 / Q4
  - overtime periods when available
- export everything into a multi-sheet Excel workbook or JSON file
...
```bash
npm run match -- matchUrl="https://www.flashscore.ua/match/basketball/cantu-4jexwVmq/milan-tUT82gR9/?mid=ngW4xold" limit=20 h2hLimit=20 fileType=xlsx
```
