...
class SuperBasketCalculator:
    ...
    def evaluate_market(self, market: dict[str, Any], canonical: dict[str, Any]) -> dict[str, Any]:
        ...
        if zone_credibility.get('available') and zone_credibility.get('cases', 0) >= int(self.config.get('credibility', {}).get('zone_matrix_min_cases', 10)):
            if zone_credibility.get('shrunk_hit_rate', 0.5) < float(self.config.get('credibility', {}).get('zone_matrix_low_threshold', 0.55)):
